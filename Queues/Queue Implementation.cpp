...
// Push from rear and pop from front
class CustomQueue
{
    int *arr;
    int front;
...
public:
    ...
    int getFront()
    {
        if (front == -1)
        {
            cout << "No lement in the Queue" << endl;
            return -1;
        }
        else
            return arr[front];
...
// Same as queue But Pop and Push in circular manner - means if there is space in the front then push from there and pop from rear and vice versa.
class CircularQueue
{
public:
    int *arr;
...
// Deque Implementation - Doubly Ended Queue
...
//Circular Deque Implementation - Doubly Ended Queue
class CircularDq
{ 
public:
    int *arr;
