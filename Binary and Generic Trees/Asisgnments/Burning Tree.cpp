...
class Solution {
  public:
    Node* makeNodeToParentMappingtAndFindTargetNode(Node* root, unordered_map<Node*, Node*>&parentMap, int target){
        queue<Node*> q;
        Node* targetNode = 0;
        q.push(root);
        parentMap[root] = 0;
    ...
    int minTime(Node* root, int target) 
    {
        ...
        Node* targetNode = makeNodeToParentMappingtAndFindTargetNode(root, parentMap, target);
